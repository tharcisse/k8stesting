...
class PosOrder(models.Model):
    ...
    def get_details(self, shop_id, order=None):
        ...
        custom_values = self.env['pos.custom.order'].get_details(shop_id)
        values['orders'] += custom_values['orders']
        values['order_lines'] += custom_values['order_lines']
        return values
    ...
    def order_progress_draft(self, id):
        ...
        custom = False
        try:
            temp_id = current_id.split("_")
            if (len(temp_id) > 1):
                current_id = temp_id[1]
                custom = True
        except Exception:
            pass
        if not custom:
            order = self.browse(int(current_id))
        else:
            order = self.env['pos.custom.order'].browse(int(current_id))
    ...
    def check_order(self, order_name):
        ...
        if not pos_order:
            pos_order = self.env['pos.custom.order'].sudo().search([('name', '=', order_name)])
